...
class Access_to_Host(object):
    ...
    def exist_buff(self, ssl_client_data):
        """
        判断客户端的请求文件是否在缓存中
        ...
        try:
            # 从数据包中解析出路径
            all_src_data = ssl_client_data.decode()
            ...
            print(e)
            return file_exist, buff_data, filename
    ...
    def send_from_buff(self, src_conn, buff_data):
        """
        ...
        try:
            ...
            for i in range(0, len(buff_data)):
                src_conn.send(buff_data[i].encode())
        except Exception as e:
            print(e)
    ...
    def ssl_client_server(self, src_conn, dst_conn):
        ...
        while True:
            ...
            print('*' * 100)
            print(ssl_client_data.decode())
            print('*' * 100)

            # 从客户端获取数据成功
            # 先去buffer中查找
            file_exist, buff_data, filename = self.exist_buff(ssl_client_data)
            if file_exist:
                # 如果buffer中有响应报文
                suss = self.send_from_buff(buff_data)
                if not suss:
                    if ssl_client_data:
                        # 发送数据给服务器
                        try:
