...
class Access_to_Host(object):
    ...
    def handler(self, conn, addr):
        ...
        # 缓存响应报文(只缓存HTTP，HTTPS只建立了连接)
        if not ssl_flag:
            self.save_in_buff(all_src_data, all_dst_data)
    ...
    def save_in_buff(self, all_src_data, all_dst_data):
        """
        保存响应报文到buffer中
        :param all_src_data: 客户端发送的报文
        :param all_dst_data: 服务器响应的报文
        :return: 是否保存成功
        """
        try:
            # 从数据包中解析出路径
